...
這個 pass 將
    ...
    * string literal 轉成 list of char
    * list literal 跟 list pattern desugar 成 cons/nil 型式

-}
module Desugar.Lit where
import Desugar.Monad
import Language.Haskell.Exts.Annotated

con l n = Con l . UnQual l . name l $ n

consN = "Prelude.Cons"
nilN  = "Prelude.Nil"

desugarLit :: Module l -> DesugarM (Module l)
desugarLit = transModule
...
transExp (Lit l literalL)
  = case literalL of
      String l s _ -> return $ foldr cons nil s
      _ -> do
              literalL' <- transLiteral literalL
              return (Lit l literalL')
  where cons x xs = appFun [l, l]
                       (con l consN)
                       [charE l x, xs]
        nil = con l nilN
...
transLiteral (String l string string1)
  = error "desugarLit: string is not valid"
...
transPat (PLit l signL literalL)
  = case literalL of
         String _ s _ -> return $ foldr cons nil s
         _ -> do
            signL' <- transSign signL
            literalL' <- transLiteral literalL
            return (PLit l signL' literalL')
  where cons x xs
              = pApp l (name l consN)
                       [charP l x, xs]
        nil = pApp l (name l nilN) []
...
transPat (PList l patLs)
  = do patLs' <- mapM transPat patLs
       return $ foldr cons nil patLs'
  where cons x xs
            | let l = ann x
              = pApp l (name l consN) [x, xs]
        nil = pApp l (name l nilN) []
