...
@
-}
module Desugar.Where (desugarWhere) where

import Desugar.Monad
import Language.Haskell.Exts.Annotated

desugarWhere :: Module l -> DesugarM (Module l)
desugarWhere = transModule
...
-- desugar: where
transAlt :: Alt l -> DesugarM (Alt l)
transAlt (Alt l patL (UnGuardedRhs rhsL exp) maybeBindsL)
  = do patL' <- transPat patL
       return (Alt l patL' (UnGuardedRhs rhsL exp) Nothing)
    where exp' = case maybeBindsL of
                       Nothing -> exp
                       Just (BDecls bsl bs) -> letE bsl bs exp

transAnnotation :: Annotation l -> DesugarM (Annotation l)
...
transAssoc :: Assoc l -> DesugarM (Assoc l)
...
transAsst :: Asst l -> DesugarM (Asst l)
...
transBooleanFormula ::
                    BooleanFormula l -> DesugarM (BooleanFormula l)
...
transBracket :: Bracket l -> DesugarM (Bracket l)
...
transCName :: CName l -> DesugarM (CName l)
...
transCallConv :: CallConv l -> DesugarM (CallConv l)
...
transClassDecl :: ClassDecl l -> DesugarM (ClassDecl l)
...
transConDecl :: ConDecl l -> DesugarM (ConDecl l)
...
transContext :: Context l -> DesugarM (Context l)
...
-- desugar: where
transDecl (PatBind l patL (UnGuardedRhs rhsL exp) maybeBindsL)
  = do patL' <- transPat patL
       return (PatBind l patL' (UnGuardedRhs rhsL exp') Nothing)
    where exp' = case maybeBindsL of
                       Nothing -> exp
                       Just (BDecls bsl bs) -> letE bsl bs exp
...
transDeclHead :: DeclHead l -> DesugarM (DeclHead l)
...
transExportSpec :: ExportSpec l -> DesugarM (ExportSpec l)
...
transExportSpecList ::
                    ExportSpecList l -> DesugarM (ExportSpecList l)
...
transFieldDecl :: FieldDecl l -> DesugarM (FieldDecl l)
transFieldDecl (FieldDecl l nameLs typeL)
  = do nameLs' <- mapM transName nameLs
       typeL' <- transType typeL
       return (FieldDecl l nameLs' typeL')

transFieldUpdate :: FieldUpdate l -> DesugarM (FieldUpdate l)
...
transImportSpec :: ImportSpec l -> DesugarM (ImportSpec l)
...
transImportSpecList ::
                    ImportSpecList l -> DesugarM (ImportSpecList l)
...
transInstDecl :: InstDecl l -> DesugarM (InstDecl l)
...
transInstHead :: InstHead l -> DesugarM (InstHead l)
...
transInstRule :: InstRule l -> DesugarM (InstRule l)
...
transKind :: Kind l -> DesugarM (Kind l)
...
transLiteral :: Literal l -> DesugarM (Literal l)
...
-- desugar: where
transMatch :: Match l -> DesugarM (Match l)
transMatch (Match l nameL patLs (UnGuardedRhs rhsL exp) maybeBindsL)
  = do nameL' <- transName nameL
       patLs' <- mapM transPat patLs
       return (Match l nameL' patLs' (UnGuardedRhs rhsL exp') Nothing)
    where exp' = case maybeBindsL of
                       Nothing -> exp
                       Just (BDecls bsl bs) -> letE bsl bs exp
...
transModule :: Module l -> DesugarM (Module l)
transModule
...
transModuleHead :: ModuleHead l -> DesugarM (ModuleHead l)
transModuleHead
...
transModuleName :: ModuleName l -> DesugarM (ModuleName l)
transModuleName (ModuleName l string)
  = return (ModuleName l string)

transModulePragma :: ModulePragma l -> DesugarM (ModulePragma l)
...
transOverlap :: Overlap l -> DesugarM (Overlap l)
...
transPXAttr :: PXAttr l -> DesugarM (PXAttr l)
transPXAttr (PXAttr l xNameL patL)
  = do xNameL' <- transXName xNameL
       patL' <- transPat patL
       return (PXAttr l xNameL' patL')

transPat :: Pat l -> DesugarM (Pat l)
...
transPatField :: PatField l -> DesugarM (PatField l)
...
transPromoted :: Promoted l -> DesugarM (Promoted l)
...
transQName :: QName l -> DesugarM (QName l)
...
transQualConDecl :: QualConDecl l -> DesugarM (QualConDecl l)
transQualConDecl
...
transQualStmt :: QualStmt l -> DesugarM (QualStmt l)
...
transRPat :: RPat l -> DesugarM (RPat l)
...
transRPatOp :: RPatOp l -> DesugarM (RPatOp l)
...
transRole :: Role l -> DesugarM (Role l)
...
transRule :: Rule l -> DesugarM (Rule l)
transRule
...
transRuleVar :: RuleVar l -> DesugarM (RuleVar l)
...
transSafety :: Safety l -> DesugarM (Safety l)
...
transSign :: Sign l -> DesugarM (Sign l)
transSign (Signless l) = return (Signless l)
transSign (Negative l) = return (Negative l)

transSpecialCon :: SpecialCon l -> DesugarM (SpecialCon l)
...
transTyVarBind :: TyVarBind l -> DesugarM (TyVarBind l)
...
transType :: Type l -> DesugarM (Type l)
