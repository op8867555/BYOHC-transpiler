...
desugarWhere :: Module l -> Module l
...
-- desugar: 把 `e where ...` 換成 `let ... in e`
transDecl (PatBind l pat (UnGuardedRhs rhsl exp) binds) =
    PatBind l (transPat pat) (UnGuardedRhs rhsl exp') Nothing where
        exp' = case binds of
                    Nothing -> exp
...
transExp (If l exp1 exp2 exp3) =
    If l (transExp exp1) (transExp exp2) (transExp exp3)
