...
module Desugar.GADT where
import Desugar.Monad
import Language.Haskell.Exts.Annotated
import Control.Arrow ((&&&))

desugarGADT :: Module l -> DesugarM (Module l)
desugarGADT = transModule
...
transDecl
  ...
  = do dataOrNewL' <- transDataOrNew dataOrNewL
       ...
       let conNames      = map (conName &&& countVars) gadtDeclLs
           DHead _l name = declHeadL
       addCons (extractName name) conNames
       ...
       return
         (GDataDecl l dataOrNewL' maybeContextL' declHeadL' maybeKindL'
            ...
            maybeDerivingL')
    where
        conName (GadtDecl srcLoc name nameTyPairs ty) = extractName name
        countVars (GadtDecl srcLoc name nameTyPairs ty) = count 0 ty where
